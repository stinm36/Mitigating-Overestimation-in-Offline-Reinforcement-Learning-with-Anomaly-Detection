...
class DaGMM(nn.Module):
    """Residual Block."""
    def __init__(self, input_dim, hidden_dim, n_gmm = 2, latent_dim=1):
        ...
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//2,hidden_dim//4)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//4,10)]
        layers += [nn.Tanh()]
        layers += [nn.Linear(10, latent_dim)]
        ...
        layers += [nn.Tanh()]
        layers += [nn.Linear(10, hidden_dim//4)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//4,hidden_dim//2)]
        layers += [nn.Tanh()]        
        ...
        layers = []
        layers += [nn.Linear(latent_dim + 2,10)]
        layers += [nn.Tanh()]        
    ...
    def compute_energy(self, z, phi=None, mu=None, cov=None, size_average=True):
        ...
        for i in range(k):
            ...
            # eigvals, _ = torch.linalg.eig(cov_k.cpu() * (2*np.pi))
...
class DaGMM_(nn.Module):
    """Residual Block."""
    def __init__(self, input_dim, n_gmm = 2, hidden_dim = 256, latent_dim=1, config = None):
        super(DaGMM, self).__init__()
        ...
        layers += [nn.Linear(hidden_dim//2,hidden_dim//4)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//4,latent_dim)]

        self.encoder = nn.Sequential(*layers)


        layers = []
        layers += [nn.Linear(latent_dim,hidden_dim//4)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//4,hidden_dim//2)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(hidden_dim//2,hidden_dim)]
        ...
        layers = []
        layers += [nn.Linear(latent_dim + 2,hidden_dim)]
        layers += [nn.Tanh()]        
        layers += [nn.Dropout(p=0.2)]        
        layers += [nn.Linear(hidden_dim,n_gmm)]
        layers += [nn.Softmax(dim=1)]
    ...
    def forward(self, x, phi=None, mu=None, cov=None, size_average=True):
